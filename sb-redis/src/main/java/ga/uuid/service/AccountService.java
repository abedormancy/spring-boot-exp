...
public class AccountService {
	 ...
	 * <b>Description:</b><br> 
	 * 生成指定数量的随机账户
	 * @param count 需要随机生成的数量
	 * @return a random Account list
	 * @Note
	 * <b>Author:</b> abeholder
	 */
	@Cacheable(value = "du", keyGenerator = "simpleKeyGenerator")
	public List<Account> generateAccounts(int count) {
		logger.info("生成了 {} 份随机账号~", count);
		List<Account> result = IntStream.rangeClosed(1, count).mapToObj(i -> {
			Account account = new Account();
			account.setId(i);
			account.setUsername(randomName() + ThreadLocalRandom.current().nextInt(1, 100));
			return account;
	...
	private static final String randomName() {
		int index = ThreadLocalRandom.current().nextInt(0, firstName.length);
